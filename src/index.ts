...
import {
  createUnityProject,
  getUnityInstallations,
  openUnityProject,
  parseUnityVersion,
} from "./modules/unity";

const run = <T>(promise: Promise<T>, text: string) => {
  return oraPromise(promise, { text });
};

const main = async () => {
  try {
    const unityInstallations = await run(
      getUnityInstallations(),
    ...
    const projectPath = await run(
      createDirectory(inputData.packageName),
      "Create directory"
    );
    await run(
      createUnityProject(unityPath, projectPath),
